...
const getAllVideoGalleries = async (req, res) => {
  try {
    ...
    res
      .status(400)
      .json({ message: `Cannot get video galleries: ${error.message}` });
  }
};
...
const getVideoGallery = async (req, res) => {
  ...
  try {
    ...
    res
      .status(400)
      .json({ message: `Cannot get video gallery: ${error.message}` });
  }
};
...
const createVideoGallery = async (req, res) => {
  ...
  try {
    ...
    res
      .status(400)
      .json({ message: `Cannot create video gallery ${error.message}` });
  }
};
...
const deleteVideoGallery = async (req, res) => {
  ...
  try {
    ...
    const deletedVideoGallery = await VideoGallery.findOneAndDelete({
      _id: id,
    });
