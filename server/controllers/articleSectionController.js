...
const getAllArticleSections = async (req, res) => {
  try {
    const articleSections = await ArticleSection.find({}).sort({
      createdAt: -1,
    });
...
const createArticleSection = async (req, res) => {
  ...
  try {
    const articleSection = await ArticleSection.create({
      title,
      image: imageFilename,
      imageClicked: imageClickedFilename,
    });
    res.status(200).json(articleSection);
  } catch (err) {
    res.status(400).json({ error: err.message });
  ...
  try {
    ...
    if (imageClicked) {
      ...
      if (currentArticleSection.imageClicked) {
        const imageClickedPath = path.join(
          "public",
          currentArticleSection.imageClicked
        );
...
const deleteArticleSection = async (req, res) => {
  ...
  try {
    ...
    const currentArticleSectionImageClicked =
      currentArticleSection.imageClicked;
    ...
    if (currentArticleSectionImageClicked) {
      const imageClickedPath = path.join(
        "public",
        currentArticleSectionImageClicked
      );
      fs.unlinkSync(imageClickedPath);
    }
